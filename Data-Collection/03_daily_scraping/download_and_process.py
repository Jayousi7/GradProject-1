...
# Output paths under the centralized 'data/' folder relative to project root
...
def run_workflow():
    ...
    for ticker in TICKERS:
        ...
        # Step 4: Save individual CSV and Parquet files in dedicated firm folder (overwriting old)
        ...
        firm_pq_path = os.path.join(firm_folder, f"{ticker}_historical.parquet")
        
        df_processed.to_csv(firm_csv_path, index=False, encoding='utf-8')
        df_processed.to_parquet(firm_pq_path, index=False)
        print(f"[+] Saved updated files: {firm_csv_path} and {firm_pq_path} (Shape: {df_processed.shape})")
    ...
    # Step 5: Merge all sheets into the final combined Master CSV & Parquet
    if combined_dfs:
        print("\n>>> CREATING FINAL COMBINED MASTER CSV & PARQUET <<<")
        ...
        # Save CSV and Parquet master files
        df_master.to_csv(COMBINED_FILE, index=False, encoding='utf-8')
        
        combined_pq_file = COMBINED_FILE.replace(".csv", ".parquet")
        df_master.to_parquet(combined_pq_file, index=False)
        
        print(f"[+++] Workflow Complete! Saved master files: {COMBINED_FILE} and {combined_pq_file}")
